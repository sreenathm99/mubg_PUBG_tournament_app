...
import me.mubg.tournament.base.Logger;
import me.mubg.tournament.model.RssItem;
import me.mubg.tournament.util.NullUtils;
    ...
    public void onBindViewHolder(RssItemsAdapter.ViewHolder holder, int position) {
        ...
        if (NullUtils.notEmpty(item.getImageUrl())) {
            Picasso.with(mContext).load(item.getImageUrl()).
                    fit()
                    .centerCrop()
                    .error(R.drawable.pos)
                    .placeholder(R.drawable.pos)
                    .into(holder.imageThumb);
        } else {
            holder.imageThumb.setImageResource(R.drawable.pos);
        }
        ...
        Logger.logv("Fetching image url: " + item.getImageUrl());
